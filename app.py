...
with st.form(key = "form"):
    ...
    with col1:
            name = st.text_input(
                label="포켓몬 이름",
                value=example_pokemon["name"] if auto_complete else None
                )
    with col2:
        types = st.multiselect(
            label="포켓몬 속성",
            options=list(type_emoji_dict.keys()),
            max_selections=2, # 속성은 2개까지만
    ...
    #submit button을 눌렀을 때 실행 될 명령, 이미지 url은 default 값 있음
    if submit:
        if not name:
        ...
        with cols[j]:
            ...
            with st.expander(label=f"**{i+j+1}. {pokemon['name']}**", expanded=True):
                ...
                if delete_button:
                    ...
                    st.rerun() # 버튼 누르면 페이지 리로드, 뒤에 버튼 클릭됨 따라서 다시 런온시켜줘야함함
